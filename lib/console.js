...
const state = {
    ...
    init: function init(nhistfile, jhistfile) {
        ...
        state.shell.on("historyReloadWanted", () => {
            readline.clearHistory();
            readline.readHistory(state.curHistFile);
        });
    ...
    online: function online(line) {
        ...
        if (state.shell.process(line)) {
            readline.addHistory(line, true /*write*/);
        }
    },
...
module.exports = {
    ...
    run: function run(nhistfile, jhistfile) {
        readline.start();
        readline.on("historyAdded", () => {
            state.shell.historyAdded();
        });
        readline.on("line", state.online);
        readline.on("complete", state.oncomplete);
        readline.on("term", state.onterm);
        process.on("SIGINT", () => {
            state.buffer = "";
            readline.sigint();
        });
