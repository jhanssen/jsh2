...
const state = {
    ...
    dirComplete: function dirComplete(data, cb, cmp) {
        if (!data.buffer.length) {
            cb();
            return;
        }

        const filter = (data, files) => {
            let ret = [];
            let dir = false;
            for (let idx = 0; idx < files.length; ++idx) {
                // console.log("filter", files[idx], files[idx].fn.substr(0, data.buffer.length), data.buffer);
                if (files[idx].fn.substr(0, data.buffer.length) == data.buffer) {
        ...
        const complete = (key, data, cb, max) => {
            let obj;
            if ((obj = state.dircache.get(key))) {
                // console.log("got it");
                let ret = filter(data, obj);
                // console.log("aha", ret);
        ...
        let slash = data.buffer.lastIndexOf('/');
        if (slash == -1) {
        ...
        complete(data.buffer.substr(0, slash + 1), data, cb);
    }
};
        ...
        if (data.start === 0) {
            ...
            if (data.end > 0) {
                ...
                case '.':
                    ...
                    state.dirComplete(data, cb, direxe);
                    return;
                default:
                    break;
                }
