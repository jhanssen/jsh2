...
const completions = {
    ...
    complete: function complete(shell, data, cb) {
        ...
        setImmediate(() => {
            ...
            let localDirComplete = function(cb, customCheck) {
                let check = (stats) => {
                    if (stats.isDirectory())
                        return Completion.Partial;
                    return Completion.Final;
                ...
                state.dirComplete(data, tok, cb, { cmp: customCheck || check, rel: relpaths, cache: state.dirrelcache });
            };
            let localDirCompletePromise = function(check) {
                return new Promise((resolve, reject) => {
                    localDirComplete((ret) => {
                        resolve(ret);
                    }, check);
                });
            };
            let ext = { tokens: tok.expanded, text: textFunc, directoryComplete: localDirCompletePromise };
